...
const Confirmation: React.FC<ConfirmationProps> = ({
  ...
  if (!bookingDetails || !service || !location || !dateTime) {
    return (
        <div className="text-center p-12">
            ...
            <button onClick={onReset} className="mt-8 px-8 py-3 bg-muzza text-gray-800 font-semibold rounded-lg shadow-md hover:bg-muzza-dark transition-colors duration-300">
  ...
  return (
    <div className="text-center py-12 px-4 animate-fade-in">
      ...
      <p className="text-gray-600 mt-2">Hemos enviado un correo de confirmación a <span className="font-semibold text-muzza-dark">{bookingDetails.email}</span>.</p>
      ...
      <div className="mt-10 text-left max-w-lg mx-auto bg-gray-50 rounded-lg border border-gray-200 p-6 space-y-4">
        ...
        <div className="pt-4 mt-4 border-t border-dashed flex justify-between font-bold text-lg">
            ...
            <span className="text-muzza-dark">${total.toFixed(2)}</span>
        </div>
      </div>

      <button onClick={onReset} className="mt-10 px-8 py-3 bg-muzza text-gray-800 font-semibold rounded-lg shadow-md hover:bg-muzza-dark transition-colors duration-300">
        Hacer una Nueva Reserva
      </button>
    </div>
  );
